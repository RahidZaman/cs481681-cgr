class CourseMetadata {
    constructor(studentName) {
        ...
        this.courseNumber = "CS F481/F681";
        this.courseName = "Computer Graphics Rendering";
        this.courseSemester = "Spring 2019";
        this.courseDesc = `In this class, we are going to investigate computer graphics rendering. Specifically, we will be learning how to produce high quality rendered graphics using techniques drawn from Ray Tracing, Real-time Rendering, and Physically Based Rendering. We want to focus on how to understand the rendering equation
        $$L_o(\\omega_i \\rightarrow \\mathbf{x}) = L_e(\\mathbf{x} \\rightarrow \\omega_o) + \\int_{\\Omega}{f_r(\\omega_i, \\omega_o)\\ L_i(\\omega_i \\rightarrow \\mathbf{x})\\ \\langle \\omega_i, \\omega_g \\rangle\\ d\\omega}$$
        and how to implement algorithms that solve each part.`;
        ...
        if (isFinite(loc)) {
            loc = parseInt(window.location.pathname.substr(loc - 1, 1));
        }
